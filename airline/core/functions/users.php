<?php
	...
	function register_f($register_data) {
		...
		mysql_query("INSERT INTO `flight_users` ($fields,`f_active` ,`f_regdate`) VALUES ($data,'1',NOW())");
 ...
 function f_login1($f_uname, $f_password) {
		$f_id = f_id_from_username($f_uname);
		$f_uname = sanitize($f_uname);
		$f_password = md5($f_password);
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `flight_users` WHERE `f_uname`= '$f_uname' AND `f_password` = '$f_password' AND `f_status` = '1'" );
		return (mysql_result($query, 0) == 1) ? $f_id : false;
	}
?>